...
const char * const lk_matlab_type_name[] = 
{
  ...
  "1-by-n uint16",  //- DEVVAR_BOOLEANARRAY
...
int TangoBinding::read_attribute (void)
{
  ...
  mxArray * argout = ::mxCreateStructArray(2, dims, 9, field_names);
  if (! argout) 
  {
    MEX_UTILS->set_error("out of memory",
                         "mxCreateStructArray failed",
  ...
  //- be sure mx_array is NULL before trying to decode attribute value
  mxArray * mx_array = 0;

  //- be sure we have some valid data to decode
  if (attr_value.has_failed() || attr_value.quality == Tango::ATTR_INVALID)
  {
    //- set main error
    MEX_UTILS->set_error(attr_value.get_err_stack());
  ...
  else if (DATA_ADAPTER->decode_attr(ddesc, attr_id, attr_value, mx_array)) 
  {
    //- push error on error stack
    std::string r = "could not read attribute " + attr_name + " on " + dev_name;
    MEX_UTILS->push_error(r.c_str(),
	                        "could not extract value from device answer",
	                        "TangoBinding::read_attribute");
    //- release allocated memory then return error
    if (mx_array) ::mxDestroyArray(mx_array);
    ::mxDestroyArray(argout);
    SET_DEFAULT_PRHS_THEN_RETURN(kError); ;
  }
  //- argout[i].value (no error from DataAdapter::decode_attr)
  ::mxSetFieldByNumber(argout, 0, 6, mx_array);

  //- save dims 
  size_t av_m = ::mxGetM(mx_array); 
  size_t av_n = ::mxGetN(mx_array); 
  ...
  //- argout.m
  mx_array = ::mxCreateDoubleMatrix(1,1,mxREAL);
  if (mx_array) ::mxGetPr(mx_array)[0] = av_m;
  ::mxSetFieldByNumber(argout, 0, 3, mx_array);

  //- argout.n
  mx_array = ::mxCreateDoubleMatrix(1,1,mxREAL);
  if (mx_array) ::mxGetPr(mx_array)[0] = av_n;
  ...
  mx_array = ::mxCreateDoubleMatrix(1,1,mxREAL);
  if (mx_array) ::mxGetPr(mx_array)[0] = 0;
  ::mxSetFieldByNumber(argout, 0, 7, mx_array);

  //- argout.error
  ::mxSetFieldByNumber(argout, 0, 8, 0);
...
int TangoBinding::read_attribute_reply (void)
{
  ...
  {
    //- if we allocated memory then release it
    if (mx_array)  
    {
                                            ...
                                            bool delete_input)
{
  ...
  argout = ::mxCreateStructArray(2, dims, 9, field_names);
  if (! argout) 
  {
    MEX_UTILS->set_error("out of memory",
                         "mxCreateStructArray failed",
                         "TangoBinding::extract_attribute_values");
    if (delete_input) delete avs;
    return kError;
  }
  ...
  for (mwSize i = 0; i < n_attrs; i++) 
  {
    ...
    //- argout[i].name
    ::mxSetFieldByNumber(argout, i, 0, ::mxCreateString((*avs)[i].name.c_str()));
    
    //- be sure mx_array is NULL before trying to decode attribute value
    mx_array = 0;

    //- be sure we have some valid data to decode
    if ((*avs)[i].has_failed() || (*avs)[i].quality == Tango::ATTR_INVALID)
    {
      //- abort on first error?
      if (this->v1_compatibility_enabled())
      {
        MEX_UTILS->set_error((*avs)[i].get_err_stack());
        std::string r = "could not read attribute " + (*avs)[i].name + " on " + ddesc->name();
        MEX_UTILS->push_error((const char *)r.c_str(),
                              (const char *)"got invalid data from device",
                              (const char *)"TangoBinding::read_attributes");
        if (delete_input) delete avs;
        ::mxDestroyArray(argout);
        return kError;
      }
    ...
    //- decode attribute value
    else if (DATA_ADAPTER->decode_attr(ddesc, ddesc->attr_exists((*avs)[i].name), (*avs)[i], mx_array)) 
    {
      //- abort on first error?
      if (this->v1_compatibility_enabled())
      {
        std::string r = "could not read attribute " + (*avs)[i].name + " on " + ddesc->name();
        MEX_UTILS->push_error((const char *)r.c_str(),
                              (const char *)"TANGO to Matlab data conversion failed",
                              (const char *)"TangoBinding::read_attributes");
        if (delete_input) delete avs;
        ::mxDestroyArray(argout);
        return kError;
      ...
      (*avs)[i].quality = Tango::ATTR_INVALID;
      //- the trick: build error by hand...
      try
      {
        std::string r = "could not read attribute " + (*avs)[i].name + " on " + ddesc->name();
        Tango::Except::throw_exception(r.c_str(), 
                                       (const char *)"TANGO to Matlab data conversion failed",
                                       (const char *)"TangoBinding::read_attributes");
      }
      ...
      {
        if (! (*avs)[i].ext)
          (*avs)[i].ext = new Tango::DeviceAttributeExt;
        (*avs)[i].ext->err_list = new Tango::DevErrorList(df.errors);
      }
      //- no valid data for this attribute
      has_failed = true;
    }
    ...
    //- save dims 
    size_t av_m = mx_array ? ::mxGetM(mx_array) : 0; 
    size_t av_n = mx_array ? ::mxGetN(mx_array) : 0; 

    //- argout[i].quality
    mx_array = ::mxCreateDoubleMatrix(1,1,mxREAL);
    switch ((*avs)[i].quality) 
    {
      case Tango::ATTR_VALID:
        if (mx_array) ::mxGetPr(mx_array)[0] = 0;
        ::mxSetFieldByNumber(argout, i, 1, mx_array);
    ...
    if (mx_array)
    {
      ::mxGetPr(mx_array)[0] = lk_time_reference 
                             ...
                             * (
                                 (*avs)[i].time.tv_sec  + 
                                 1.E-6 * (*avs)[i].time.tv_usec + 
                                 1.E-9 * (*avs)[i].time.tv_nsec
                               );
    }
    ...
    //- argout[i].has_failed
    mx_array = ::mxCreateDoubleMatrix(1,1,mxREAL);
    if (mx_array) ::mxGetPr(mx_array)[0] = has_failed ? 1. : 0.;
    ::mxSetFieldByNumber(argout, i, 7, mx_array);

    //- argout[i].error
    if (! has_failed) 
    {
      //- no error: argout[i].error = NULL
      ::mxSetFieldByNumber(argout, i, 8, 0);
    }
    else 
    {
      //- fill argout[i].error 
      const char * errors_field_names[] = {"reason", "desc", "origin", "severity"};
      const Tango::DevErrorList & errors = (*avs)[i].get_err_stack();
      int n_errors = errors.length();
      if (n_errors)
      {
        //- error comes from Tango side
        //-----------------------------------------------------------------------------
        mwSize errors_dims[2] = {1, n_errors};
        mx_array = ::mxCreateStructArray(2, errors_dims, 4, errors_field_names);
        if (mx_array) 
        {
          //- populate the error struct array
...
/*
  ...
  int len = dahl->size();
  mwSize dims[2] = {1, len};
  const char * main_field_names[] = {"value", "has_failed", "error"};
...
//***************************************************************
//-to do: adopt read_attribute(s) impl - see their impl
//***************************************************************
    //- decode attr value
    sub_mx_array = 0;
...
int TangoBinding::group_device_list (void)
{
  ...
  const mwSize dims[2] = {1, num_dev_in_group};
  mxArray * argout = ::mxCreateCellArray(2, dims);
  if (argout == 0) 
  {
    MEX_UTILS->set_error("out of memory",
...
int TangoBinding::group_set_timeout_msecs (void)
{
 //- get group id from the 2nd input arg
  ...
  bool tmo = (int)::mxGetPr(mx_tmo)[0];

  g->set_timeout_millis(tmo);
...
int TangoBinding::group_read_attributes_reply (void)
{
  ...
  for (size_t r = 0; r < arl.size(); r++) 
  {
    ...
    else 
    {
      if (arl[r].group_element_enabled() == false)
      {
        //- set undefined fields of struct attr_values_array[attr_values_idx] to NULL
        for (int f = 4; f < 11; f++)
