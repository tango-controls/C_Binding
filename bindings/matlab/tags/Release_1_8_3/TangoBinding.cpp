...
int TangoBinding::command_inout_reply (void)
{
  ...
  //- convert from C to STD string then release memory
...
int TangoBinding::read_attribute (void)
{
  ...
  const mwSize dims[2] = {1, 1};
  const char *field_names[] = {"name", "quality", "quality_str", "m", "n", "time", "value", "has_failed"};
  ...
  if (attr_value.has_failed())
  {
    ...
    MEX_UTILS->push_error(r.c_str(),
	                       "got invalid data from device (has_failed flag set to true)",
	                       "TangoBinding::read_attribute");
    SET_DEFAULT_PRHS_THEN_RETURN(kError); 
  }
...
int TangoBinding::read_attributes (void)
{
  ...
  for (i = 0; i < len; i++) 
  {
    ...
    mx_array = 0;
    //- be sure we have some valid data to decode
    if ((*attr_values)[i].has_failed())
    {
      //- no valid data for this attribute
      has_failed = true;
    }
    //- decode attribute value
    else if (DATA_ADAPTER->decode_attr(ddesc, attr_ids[i], (*attr_values)[i], mx_array)) 
    {
      //- if we allocated memory then release it
      if (mx_array)  
      {
      ...
      (*attr_values)[i].quality = Tango::ATTR_INVALID;
      //- push error on error stack
    ...
    //- argout[i].has_failed
    mx_array = ::mxCreateLogicalMatrix(1, 1);
    ::mxGetPr(mx_array)[0] = has_failed;
    ...
    //- set dims
    Tango::AttributeDimension r_dim = {0, 0};
    if (! has_failed)
      (*attr_values)[i].get_r_dimension();
    //- argout[i].m
    mx_array = ::mxCreateDoubleMatrix(1,1,mxREAL);
    ::mxGetPr(mx_array)[0] = r_dim.dim_y;
    ::mxSetFieldByNumber(argout, i, 3, mx_array);
...
int TangoBinding::group_read_attributes_reply (void)
{
  ...
  for (size_t r = 0, cdn = -1; r < arl.size(); r++) 
  {
