...
{
  //- client counter (for client requested export to same igor path)
  size_t ref_count;
  //- polling period for a given client
  unsigned long pp;
...
protected:
  ...
  //- any client?
  bool has_registered_clients (void);
  ...
  unsigned long num_ms_to_polling_period_expiration (void);
...
protected:
  ...
  void attach_error ();
  void attach_error (const Tango::DevFailed & df);
...
protected:
  ...
  bool has_monitored_objects (void) const;
