...
Structure AttributeValue
 ...
 //- when used to write an attr: not used use val_path to specify where the wave is located
 //- undefined in case of error during attribute reading - it means that you 
 //- should always check the error code returned by tango_read_attr 
 //- before trying to access this variable
 //-------------------------------------------------------------------------
...
Structure AttributeValues
 ...
 int16 nattrs
 //- full path to the data folder in which the returned waves should be placed (if any)
 String df
...
// tango_init_attr_val
//==============================================================================
function tango_init_attr_val (attr_val_io, dev, attr)
...
function tango_read_attr (attr_val_io)
 Struct AttributeValue &attr_val_io
 String cur_df
 Variable df_specified = 0 
 if (cmpstr(attr_val_io.val_path, "") == 0 && strsearch(attr_val_io.val_path, "root:", 0) != -1)
   df_specified = 1
 endif
 if (GetRTError(1))
   attr_val_io.val_path = ""
 ...
 if (df_specified == 0)
   cur_df = tango_enter_tmp_df(attr_val_io.dev)
 else
   cur_df = tango_enter_df(attr_val_io.val_path)
 endif
 ...
 switch (format)
   case kSCALAR:
 	  switch(type)
 		...
 		break
  default:
    if (tango_write_attribute(attr_val_io.dev, attr_val_io.attr, attr_val_io.val_path) == -1)
 	   tango_leave_tmp_df(cur_df)
      return -1
 	 endif
 	 break
...
function tango_read_attrs (attr_vals_io)
 ...
 String cur_df
 Variable df_specified = 0 
 if (cmpstr(attr_vals_io.df, "") == 0 && strsearch(attr_vals_io.df, "root:", 0) != -1)
   df_specified = 1
 endif
 if (GetRTError(1))
   attr_vals_io.df = ""
 ...
 if (df_specified == 0)
   cur_df = tango_enter_tmp_df(attr_vals_io.dev)
 else
   cur_df = tango_enter_df(attr_vals_io.df)
 endif
