...
//  CONSTS
//==============================================================================
strconstant kDSERVER_PREFIX = "dserver/"
strconstant kSTARTER_PREFIX = "tango/admin/"
strconstant kSYSTEM_PREFIX  = "sys/"
...
function tp_build() 
	...
	ListBox dev_list, pos={15,34}, size={249,310}, frame=2
	...
	CheckBox had_cb, pos={45,14}, size={193,15}, proc=tp_ctrlproc_hadcb
	CheckBox had_cb, font="Courier New", title="Hide TANGO admin. devices"
	CheckBox had_cb, value=1
	tp_disable_items()
...
//==============================================================================
// fonction : tp_ctrlproc_hadcb
//==============================================================================
Function tp_ctrlproc_hadcb (ctrlName, checked) : CheckBoxControl
	String ctrlName
	Variable checked
	tp_ctrlproc_update_list_but ("dummy")
End
...
function tp_ctrlproc_tab (name, tab)
	...
	String ldf
	if (tango_enter_device_df(list[V_Value], ldf) == -1)
	  return -1
	endif
	switch (tab)
		case 0:
...
function tp_ctrlproc_update_list_but (ctrlName) : ButtonControl
   ...
   if (tango_command_inout(tango_host, "DbGetDeviceExportedList", wc, temp_var) == -1)
	  tango_display_error()
	...
	ControlInfo /W=tango_panel had_cb
	Variable hide_admin_devices = V_Value
	if (hide_admin_devices)
...
function tp_ctrlproc_device_listbox (ctrl_name, row, col, event)
	...
	String ldf
	if (tango_enter_device_df(list[row], ldf) == -1)
	  tp_device_error(row)
	  return -1
	endif
	Variable err = 0
...
//	Make /O /T /N=(Dimsize(alist, 0)) tmp_attr_name = alist[x][0]
//	Make /O /N=(Dimsize(alist, 0)) index
//	MakeIndex /A tmp_attr_name, index
//	Duplicate /O /T alist, tmp_attr_list
//	Variable i
//	Variable max_i = Dimsize(alist, 0) 
//	for (i = 0; i < max_i; i += 1)
//	  alist[i][q] = tmp_attr_list[index[i]][q]
//	endfor	
// KillWaves tmp_attr_name, tmp_attr_list, index 
