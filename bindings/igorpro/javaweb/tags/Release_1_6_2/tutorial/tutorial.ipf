...
// typing "tango_test_all(n_it)" - however, in the case, the name of the running
// TangoTest device should be "tango/tangotest/1".
//
//  Table of Content :
//  ------------------
...
// argin while <CmdArgOut> represents argout. These two structures are defined
// in the <Tango.ipf> file.
//
// <CmdArgIn> has the following members:
// -------------------------------------
//  Structure CmdArgIn
...
function test_tango_cmd_io (dev_name)
  ...
  print "\r<" + argin.cmd + "> cmd passed\r"
...
function test_tango_read_attrs (dev_name)
 ...
 //- read in a dedicated string in which attributes names are separated by a ';'.
 ...
 //- AttributeValues struct per device instead. Unfortunatly, Igor structs can't "live"
 //- outside the context of a function. It means that we can't create a global struct (i.e.
 //- Sruct/G does not work). Consequently, we have to rebuild the reading structur each
 //- time we want to read the attributes from the background task. Maintaining the attribute
...
function test_tango_write_spec_or_img (dev_name, ro_attr, rw_attr)
  ...
  if ((rd_wave[x] != wt_wave[x]) && (WaveType(rd_wave) & 0x2 == 0))
