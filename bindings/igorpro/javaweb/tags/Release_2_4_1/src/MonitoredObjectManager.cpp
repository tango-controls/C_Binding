...
#if defined (_LOG_)
    XDK_UTILS->notify("Tango-Binding::adding monitor for %s/%s", dev_name.c_str(), attr_name.c_str());  
#endif
                                              ...
                                              long client_id)
{
  ...
  if (this->mdev_map_.empty())
  {
    ...
    this->enable_periodic_msg(false);
#if defined (_LOG_)
    XDK_UTILS->notify("Tango-Binding::no more active monitor [periodic msg disabled]");  
#endif 
  }
  else
  {
  ...
  try
  {
	  ...
	  switch (_msg.type())
	  {
		  ...
		  case yat::TASK_INIT:
			  {
#if defined (_LOG_)
          XDK_UTILS->notify("MonitoredObjectManager::handle_message::TASK_INIT"); 
#endif
		  ...
		  case yat::TASK_EXIT:
			  {
#if defined (_LOG_)
          XDK_UTILS->notify("MonitoredObjectManager::handle_message::TASK_EXIT"); 
#endif
...
void MonitoredObjectManager::import_values ()
{
  //- enter critical section
  //
  //- why do we have to <import values> from devices under critical section?
  //------------------------------------------------------------------------
  //- because the MonitoredDeviceMap (i.e. this->mdev_map_) could be modified  
  //- while iterating on its content (add/remove attribute monitor)
  yat::AutoMutex<yat::Mutex> guard(this->m_lock);
...
void MonitoredObjectManager::export_values ()
{
  //- enter critical section
  //-
  //- why do we have to <export_values> to Igor under critical section?
  //-------------------------------------------------------------------
  //- <MonitoredObjectManager::export_values> is triggered by Igor's <IDLE> msg
  //- (see XDK_Xop::process_igor_message & TangoClientXop::idle) - as far as
  //- Igor is single threaded (i.e. can't "really" execute to actions at the
  //- "same" time) - the MonitoredDeviceMap (i.e. this->mdev_map_) can't be 
  //- modified while iterating on its content (no add/remove attribute monitor)
  yat::AutoMutex<yat::Mutex> guard(this->m_lock);
