                              ...
                              Tango::DeviceData& dd_out_)
{
  ...
  */
  ...
  {
    XOP_ERROR(("DataCodec::decode_argout::get_obj_df failed"));
    XDK_UTILS->set_error("XOP internal error",
                         "XDK_Utils::get_obj_df failed",
                         "DataCodec::decode_argout");
    return kError;
  ...
  try 
  {
    ...
    switch (argout_type) 
    {
      ...
      case Tango::DEV_STATE: 
      {
        Tango::DevState ds;
        dd_out_ >> ds;
        if (XDK_UTILS->create_df_obj(dfh, obj_name, static_cast<long>(ds))) 
        {
          std::string d = "failed to set global variable <" + _argout + ">";
          d += " (object name or type conflict)";
          XDK_UTILS->set_error("invalid argout specified",
                            ...
                            bool _export_nan_if_invalid)
{
  ...
  try 
  {
    ...
    switch (attr_format) 
    {
      ...
      case Tango::SCALAR: 
      {
        if (value_.has_failed() || (value_.quality == Tango::ATTR_INVALID) || value_.is_empty())
        {
#if defined (_VERBOSE_)
          std::cout << "DataCodec::decode_attr::"
                    << _ddesc->name()
                    << "/"
                    << attr_list[_attr_id].name
                    << ": value has-failed|invalid|empty"
                    << std::endl;
          std::cout << "DataCodec::decode_attr::setting "
                    << obj_name
                    << " to NAN"
                    << std::endl;
#endif 
          if (attr_type != Tango::DEV_STRING)
            XDK_UTILS->create_df_obj(dfh, obj_name, DOUBLE_NAN);  
          else
            XDK_UTILS->create_df_obj(dfh, obj_name, "Error Reading Attribute");
          if (_export_ts)
          {
            this->export_ts(obj_name, dfh, tstamp); 
            this->export_qlt(obj_name, dfh, value_.quality); 
        ...
        switch (attr_type) 
        {
          ...
          case Tango::DEV_STATE: 
          {
            Tango::DevState s;
            value_ >> s;
            err = XDK_UTILS->create_df_obj(dfh, obj_name, static_cast<long>(s));  
            if (err) 
            {
              std::string d = "failed to set global variable <" + _argout + ">";
              d += " (object name or type conflict)";
          ...
          case Tango::DEV_BOOLEAN: 
          {
            bool b;
            value_ >> b;
            err = XDK_UTILS->create_df_obj(dfh, obj_name, static_cast<long>(b ? 1 : 0));  
            if (err) 
            {
              std::string d = "failed to set global variable <" + _argout + ">";
              d += " (object name or type conflict)";
          ...
          case Tango::DEV_UCHAR: 
          {
            unsigned char uc;
            value_ >> uc;
            err = XDK_UTILS->create_df_obj(dfh, obj_name, static_cast<long>(uc));  
            if (err) 
            {
              std::string d = "failed to set global variable <" + _argout + ">";
              d += " (object name or type conflict)";
          ...
          case Tango::DEV_USHORT: 
          {
            unsigned short us;
            value_ >> us;
            err = XDK_UTILS->create_df_obj(dfh, obj_name, static_cast<long>(us));  
            if (err) 
            {
              std::string d = "failed to set global variable <" + _argout + ">";
              d += " (object name or type conflict)";
          ...
          case Tango::DEV_LONG: 
          {
            long l;
            value_ >> l;
            err = XDK_UTILS->create_df_obj(dfh, obj_name, l);  
            if (err) 
            {
              std::string d = "failed to set global variable <" + _argout + ">";
              d += " (object name or type conflict)";
          ...
          case Tango::DEV_FLOAT: 
          {
            float f;
            value_ >> f;
            err = XDK_UTILS->create_df_obj(dfh, obj_name, static_cast<double>(f)); 
            if (err) 
            {
              std::string d = "failed to set global variable <" + _argout + ">";
              d += " (object name or type conflict)";
          ...
          case Tango::DEV_DOUBLE: 
          {
            double d;
            value_ >> d;
            err = XDK_UTILS->create_df_obj(dfh, obj_name, d); 
            if (err) 
            {
              std::string d = "failed to set global variable <" + _argout + ">";
              d += " (object name or type conflict)";
...
int DataCodec::export_ts (const std::string& name, DFHndl dfh, double tstamp)
{
  ...
  int err = XDK_UTILS->create_df_obj(dfh, ts_obj_name, tstamp);  
  if (err) 
  {
...
int DataCodec::export_qlt (const std::string& name, DFHndl dfh, Tango::AttrQuality qlt)
{
  ...
  int err = XDK_UTILS->create_df_obj(dfh, qlt_obj_name, static_cast<double>(qlt));  
  if (err) 
  {
    std::string d = "failed to set global variable <" + name + ">";
    d += " (object name or type conflict)";
