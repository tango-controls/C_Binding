...
int compute_image_projection_impl (T_ImgProjStruct * xfp, T = 0)
{
  ...
  long src_ndims;
  long src_dims[MAX_DIMENSIONS + 1];
  ...
  //- dest wave dims
  long dest_dims[MAX_DIMENSIONS + 1];
  ::memset(dest_dims, 0, (MAX_DIMENSIONS + 1) * sizeof(long));

  //- x & y range
  size_t x_min = xfp->roi_x0; 
  size_t x_max = xfp->roi_x1;  
  size_t y_min = xfp->roi_y0;  
  size_t y_max = xfp->roi_y1;  
