                                           ...
                                           const std::string& _iop)
{
  ...
  XDK_UTILS->notify("Tango-Binding::MonitoredObject::register_client::client exists for same dev/attr/export-path - ref.count:%d - returning existing cid", it->second.ref_count);  
#endif
      return it->first;
    }
  }
...
#if defined (_XDK_DEBUG_)
  XDK_UTILS->notify("Tango-Binding::MonitoredObject::unregister_client - ref.cpount:%d", it->second.ref_count);  
#endif
...
#if defined (_XDK_DEBUG_)
  XDK_UTILS->notify("Tango-Binding::MonitoredObject::unregister_client - no more clients - removing item");  
#endif
...
{
  this->error_counter_++;
  this->value_.quality = Tango::ATTR_INVALID;
  this->updated_ = true;
  _GET_TIME(this->import_ts_);
}

// ============================================================================
// MonitoredAttribute::attach_error
// ============================================================================
void MonitoredAttribute::attach_error (const Tango::DevFailed & _df)
{
  this->error_counter_++;
  this->error_list_ = _df.errors;
  this->value_.quality = Tango::ATTR_INVALID;
  this->updated_ = true;
  _GET_TIME(this->import_ts_);
}
...
void MonitoredAttribute::export_value ()
{
  ...
  for (; it != end; ++it)
  {
    if (! it->second.suspended && _ELAPSED_MSEC(it->second.export_ts, now) >= (0.9 * it->second.pp))
    {
//-#if defined (_XDK_DEBUG_)
//-  XDK_UTILS->notify("Tango-Binding::exporting monitor value for %s/%s to %s", 
//-                     this->tango_dev_name_.c_str(),
//-                     this->tango_obj_name_.c_str(),
...
//-#endif
      ...
      {
        XDK_UTILS->push_error("failed to export attribute value", 
      ...
      _GET_TIME(it->second.export_ts);
    }
  }

  //- mark value as obsolete
  this->updated_ = false;
}
...
{
#if defined (_XDK_DEBUG_)
  XDK_UTILS->notify("Tango-Binding::adding %s to %s monitored attribute list", 
                     attr_name.c_str(), 
                     this->tango_dev_name_.c_str());  
#endif
...

  {
    ...
    {
#if defined (_XDK_DEBUG_)
      XDK_UTILS->notify("Tango-Binding::%s is already monitored - registering new client...",  obj.c_str());  
#endif
...
// ============================================================================
MonitoredDevice::remove_attribute (const std::string& attr_name,
                                   long client_id)
{
#if defined (_XDK_DEBUG_)
    XDK_UTILS->notify("Tango-Binding::about to remove monitored attribute <%s> for client %d on device %s", 
                       attr_name.c_str(), 
                       client_id, 
                       this->tango_dev_name_.c_str());  
#endif
...
#if defined (_XDK_DEBUG_)
    XDK_UTILS->notify("Tango-Binding::removed all monitored attributes for client %d on device %s", 
                      client_id, 
                      this->tango_dev_name_.c_str());  
#endif
  }
  ...
  {
#if defined (_XDK_DEBUG_)
    XDK_UTILS->notify("Tango-Binding::removing %s from %s monitored attribute list for client %d", 
                      attr_name.c_str(), 
                      this->tango_dev_name_.c_str(), 
                      client_id);  
#endif
    ...
    if (it != this->mattr_map_.end()) 
    {
...
#endif
#if defined (_XDK_DEBUG_)
        XDK_UTILS->notify("Tango-Binding::%s removed from %s monitored attributes list",  
                          attr_name.c_str(), 
                          this->tango_dev_name_.c_str());  
#endif
      }
      ...
      {
#if defined (_XDK_DEBUG_)
        XDK_UTILS->notify("Tango-Binding::monitored attribute %s/%s has client (kept in list)", 
                          attr_name.c_str(), 
                          this->tango_dev_name_.c_str());  
#endif      
      }
