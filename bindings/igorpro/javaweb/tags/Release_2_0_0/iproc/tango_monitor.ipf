...
static function tmon_kill (win_name, [kill_win])
	...
	String ldf = tango_enter_attr_df(wns.dev, wns.attr)
...
function tmon_scalar_func (val, val_path, dat_wav_path, tms_wav_path, win_name)
   ...
   Variable i = numpnts(dat_wav) - 1
	if (numtype(val) == 2)
		SetDrawLayer /W=$win_name ProgFront
	...
	if (last_qlt != qlt)
		...
		last_qlt = qlt
		SetDrawLayer/K /W=$win_name ProgFront
	endif
	return kNO_ERROR
end
//==============================================================================
...
static function tmon_display_image (win_name, win_title, img, img_path)
	...
	TabControl tab_ctrl,win=$win_name,font="MS Sans Serif",fSize=8
	...
	Button snapshot_but,win=$win_name,proc=tmon_but_proc_snapshot,title="Snapshot"
...
function tmon_but_proc_pause (ba)
	...
	Struct TMonWinNote wns
	tmon_get_win_note(ba.win, wns)
	if (tango_enter_device_df(wns.dev, ldf) == kERROR)
		return kERROR
	endif
...
function tmon_start_dev_state_monitor (dev, cid, state_path)
	...
	String cur_df = GetDataFolder(1)
	String dev_df = tango_enter_attr_df(dev, "State")
...
function tango_dp (dev_name)
	...
	//- standalone monitor
	Button stdln_mon_but, win=$win_name, pos={235,24}, size={140,20}, title="Open Standalone Monitor"
