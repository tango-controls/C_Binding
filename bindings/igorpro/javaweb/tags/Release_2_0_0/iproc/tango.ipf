...
function tango_cmd_inout (dev, cmd, [arg_in, arg_out])
	...
	String cur_df = tango_enter_tmp_df(dev)
...
function tango_print_error ([print_full_stack])
	...
	if (WaveExists(error_stack) && dimsize(error_stack, 0))
		if (print_full_stack)
			Variable i
			print "Tango-Binding::ERROR REPORT ---------------------------------"
...
function/S tango_enter_attr_df  (dev, attr)
	...
	if (DataFolderExists(attr_df) == 0)
		attr_df  = tango_get_attr_df_by_name(dev, attr)
		if (strlen(attr_df) == 0)
...
//==============================================================================
function/S tango_get_attr_df_by_name (dev, attr)
	String dev
	String attr
	String ldf = tango_enter_attrs_df(dev)
	Variable index = 0
	String attr_df
	String dev_attrs_df = GetDataFolder(1)
	do
		attr_df = GetIndexedObjName(dev_attrs_df, 4, index)
		if (strlen(attr_df) == 0)
			break
		endif
		String str_name_path =  dev_attrs_df + attr_df + ":name"
...
//==============================================================================
function/S tango_get_cmd_df_by_name (dev, cmd)
	String dev
	String cmd
	String ldf =  tango_enter_cmds_df(dev)
	Variable index = 0
	String cmd_df
...
Function tango_save_prefs ([prefs])
	...
	if (ParamIsDefault(prefs))
		...
		Struct TangoBindingPrefs actual_prefs
		actual_prefs.version = 100
		actual_prefs.tango_host = tango_host
		actual_prefs.tmon_default_pp = tmon_default_pp
	else
