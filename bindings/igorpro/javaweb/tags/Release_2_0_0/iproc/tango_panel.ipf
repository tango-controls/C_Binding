...
function tp_build() 
   ...
   if (strlen(wl))
	...
	TabControl tab,win=tango_panel,font="MS Shell Dlg", fsize=kLB_FONTSIZE
	...
	ListBox dev_list,win=tango_panel,font="MS Shell Dlg", fsize=(kLB_FONTSIZE + 2)
	ListBox dev_list,win=tango_panel,userColumnResize=1
	ListBox err_list,win=tango_panel,pos={344,102},size={580,293}, frame=2
	ListBox err_list,win=tango_panel,mode=2, font="MS Shell Dlg", fsize=kLB_FONTSIZE
	...
	ListBox cmd_list,win=tango_panel,mode=2, font="MS Shell Dlg", fsize=kLB_FONTSIZE
	...
	ListBox attr_list,win=tango_panel,mode=2, font="MS Shell Dlg", fsize=kLB_FONTSIZE
	...
	ListBox bb_list,win=tango_panel,mode=2, font="MS Shell Dlg", fsize=kLB_FONTSIZE,listwave=empty_list
	ListBox bb_list,win=tango_panel,userColumnResize=1
	ListBox prop_list,win=tango_panel,pos={344,102},size={580,293}, frame=2
	ListBox prop_list,win=tango_panel,mode=2, font="MS Shell Dlg", fsize=kLB_FONTSIZE
	...
	Button dev_update_but,win=tango_panel,proc=tp_ctrlproc_update_list_but
	SetVariable version_sv,win=tango_panel,pos={334,15},size={80,16}, title="Version"
	SetVariable version_sv,win=tango_panel,font="MS Shell Dlg", limits={0,0,0}, noedit=1
	SetVariable class_sv,win=tango_panel,pos={423,15},size={150,16}, title="Class"
	SetVariable class_sv,win=tango_panel,font="MS Shell Dlg", limits={0,0,0}, noedit=1
	SetVariable url_sv,win=tango_panel,pos={335,42},size={540,16}, title="URL"
	SetVariable url_sv,win=tango_panel,font="MS Shell Dlg", limits={0,0,0}, noedit=1
	SetVariable host_sv,win=tango_panel,pos={581,15},size={352,16}, title="Host"
	SetVariable host_sv,win=tango_panel,font="MS Shell Dlg", limits={0,0,0}, noedit= 1
	...
	CheckBox sa_cb,win=tango_panel,font="MS Shell Dlg", title="Show Aliases",value=1
	CheckBox sc_cb,win=tango_panel,pos={180,14},size={84,14},proc=tp_ctrlproc_sccb
	CheckBox sc_cb,win=tango_panel,font="MS Shell Dlg",title="Show Classes",value=0
	...
	SetVariable wild_card,win=tango_panel,font="MS Shell Dlg", proc=tp_ctrlproc_wc
...
function tp_ctrlproc_tab (name, tab)
	...
	String ldf
	if (tango_enter_device_df(list[V_Value][1], ldf) == -1)
		return kERROR
	endif
	switch (tab)
		case kTAB_CMD:
...
function tp_ctrlproc_update_list_but (ctrlName) : ButtonControl
	...
	String ldf
	do
		if (tango_enter_device_df(tango_host, ldf) == -1)
			SetDrawLayer /W=tango_panel /K UserFront
			tp_show_error()
	...
	switch (lba.eventCode)
		case 1:
			if (lba.eventMod & 0x10)
				...
				popup_str += "Open Device Panel;\M1-;"
				if (! xptm)
					popup_str += "\M1("
				endif
				...
				switch (V_Flag)
					case 1:
						...
						break
					case 3:
						tango_dp(lba.listWave[lba.row][1])
						break
					case 5:
				...
				String ldf
				if (tango_enter_device_df(lba.listWave[lba.row][1], ldf) == -1)
					SetDrawLayer /W=tango_panel /K UserFront
...
function tp_ctrlproc_cmd_listbox (lba)
	...
	String ldf = tango_enter_tmp_df (dev)
...
function tp_update_cmd_list (device_name)
	String device_name
	String ldf = tango_enter_cmds_df(device_name)
...
function tp_update_bb_list (device_name)
	String device_name
	String ldf = tango_enter_tmp_df(device_name)
...
function tp_update_prop_list (device_name)
	...
	if (num_prop)
		...
		String cur_df
		tango_enter_device_df(device_name, cur_df)
