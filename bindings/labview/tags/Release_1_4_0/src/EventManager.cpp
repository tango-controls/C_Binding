  ...
  throw (Tango::DevFailed)
{
  ...
  try 
  {
    DeviceDesc * ddesc = DEV_REP->device_desc(this->dev_);
...
{ 
  ...
  DeviceDesc * ddesc = DEV_REP->device_desc(it->second->device_name());
  if (ddesc == 0) 
  {
    Tango::Except::throw_exception(_CPTC("internal error [unexpected null DeviceDesc]"),
			                             _CPTC("failed to unsubscribe to Tango::event"),
			                             _CPTC("EventManager::unsubscribe"));
  }
