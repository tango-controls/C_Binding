...
LVB_EXPORT long group_write_attributes (long, void*, void*); 
                 ...
                 void* error_stack_)
{
  try
  {
    ...
    *group_size_ = gdesc->device_list().size();
  }
  catch (const Tango::DevFailed& e)
  {
    ERROR_STACK->devfailed(e, _TO_LV_ERR_STACK(error_stack_));
...
//=============================================================================
// BINDING C-INTERFACE TO LABVIEW: group_write_attributes
//=============================================================================
                            ...
                            void* error_stack_)
{
  ...
  try
  {
    ...
    //- check returned reference
    if (! gdesc || ! gdesc->group() == 0)
    {
      Tango::Except::throw_exception(_CPTC("invalid group identifier"),
                                     _CPTC("invalid group identifier specified [no such group]"),
                                     _CPTC("group_write_attributes"));
    } 

    //- get a device supporting the "target attribute"
    //- group device list
    gdesc->device_list()
    ...
    if (n == 1)
    {
      //- get attribute value reference
      LvAttributeValuePtr av = (*avah)->attr_values;
      //- get attr name
      LvStringHdl sh = reinterpret_cast<LvStringHdl>(av->attr_name);
      attr_name.assign(reinterpret_cast<char*>((*sh)->data), 
                       reinterpret_cast<char*>((*sh)->data) + (*sh)->length);
...
long set_timeout (const char* _dev_name, unsigned long _tmo, void* error_stack_)
{
  try
  {
    //- avoir too short tmo (min = 1 second)
